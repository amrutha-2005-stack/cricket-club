...
type VideoItem = { id: string; title: string; category: string | null; year: number | null; order: number };
...
export default function VideosAdminPage() {
  ...
  async function handleMove(index: number, direction: -1 | 1) {
    ...
    if (!target) return;
    await Promise.all([
      fetch(`/api/admin/videos/${current.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order: target.order }) }),
      fetch(`/api/admin/videos/${target.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order: current.order }) }),
    ]);
    load();
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl font-extrabold text-navy-950">Videos</h1>
        <Link href="/admin/dashboard/videos/new" className="rounded-md bg-navy-950 px-4 py-2.5 text-sm font-bold text-white hover:bg-navy-800">
          + Add Video
        </Link>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-xl bg-white shadow-sm">
        {loading ? (
          ...
          <p className="p-6 text-sm text-navy-950/50">No videos added yet.</p>
        ) : (
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={item.id} className="border-b border-navy-950/5 last:border-0">
                  ...
                  <td className="p-4 text-navy-950/60">{item.category ?? "—"}</td>
                  ...
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      ...
                      <Link href={`/admin/dashboard/videos/${item.id}`} className="rounded border border-navy-950/15 px-3 py-1 font-semibold hover:bg-navy-950/5">Edit</Link>
