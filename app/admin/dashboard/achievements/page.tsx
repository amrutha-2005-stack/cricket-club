...
export default function AchievementsAdminPage() {
  ...
  async function handleMove(index: number, direction: -1 | 1) {
    ...
    await Promise.all([
      fetch(`/api/admin/achievements/${current.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ order: target.order }),
      }),
      fetch(`/api/admin/achievements/${target.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl font-extrabold text-navy-950">Achievements</h1>
        <Link href="/admin/dashboard/achievements/new" className="rounded-md bg-navy-950 px-4 py-2.5 text-sm font-bold text-white hover:bg-navy-800">
          + Add Achievement
        </Link>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-xl bg-white shadow-sm">
        {loading ? (
          ...
          <p className="p-6 text-sm text-navy-950/50">No achievements added yet.</p>
        ) : (
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={item.id} className="border-b border-navy-950/5 last:border-0">
                  ...
                  <td className="p-4 text-navy-950/60">{item.tournamentName ?? "—"}</td>
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      ...
                      <Link href={`/admin/dashboard/achievements/${item.id}`} className="rounded border border-navy-950/15 px-3 py-1 font-semibold hover:bg-navy-950/5">
                        Edit
                      </Link>
                      <button onClick={() => handleDelete(item.id)} className="rounded border border-red-200 px-3 py-1 font-semibold text-red-600 hover:bg-red-50">
                        Delete
                      </button>
                    </div>
                  </td>
